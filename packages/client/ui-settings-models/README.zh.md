...
`dsh-client-ui-settings-models` 是 dsh Web 客户端的 Models 设置页面：用户可以配置 API 密钥（以只写方式存入 profile 的凭据引用之下）、编辑每个提供方的模型列表、选择全局默认模型，并手工声明自定义 pi-ai 路由。该页面把提供方目录、实时模型目录、设置文档与凭据描述合并为一个共享快照。它还会带首次运行的用户走两个有序弹窗——版本化内测声明，以及按条件显示的官方 DeepSeek 凭据步骤。
...
页面顶部基于实时目录的选择器会显示当前 DSH 默认模型以及所有已公布的模型路由。点击**设为默认**会把 `provider` 与 `model` 写入既有的 `agent-default-model` 设置分节，并清除之前保存的推理强度，避免旧强度被应用到不兼容模型。新会话以及读取 `ctx.agentDefaultModel` 的 Host 功能会使用保存后的选择；已有请求记录的会话仍保留自己的持久化选择。
...
无。该包只编辑其他 DSH 服务应使用哪条模型路由，本身不发送模型请求。
